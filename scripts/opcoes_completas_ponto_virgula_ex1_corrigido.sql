-- Script para adicionar opções a TODAS as questões do Exercício 1 de Ponto e Vírgula (CORRIGIDO)
...
BEGIN
    ...
    -- Corrigido para não usar ORDER BY com array_agg
    SELECT array_agg(id) INTO exercicios_ids
    FROM exercises
    WHERE category_id = 'ponto-virgula';
    
    -- Ordenar o array para garantir que os exercícios estejam em ordem
    SELECT array(SELECT unnest(exercicios_ids) ORDER BY 1) INTO exercicios_ids;
...
BEGIN
    -- Obter todos os IDs dos exercícios da categoria ponto-virgula
    -- Usando abordagem diferente para obter os IDs ordenados
    SELECT array(
        SELECT id 
        FROM exercises 
        WHERE category_id = 'ponto-virgula' 
