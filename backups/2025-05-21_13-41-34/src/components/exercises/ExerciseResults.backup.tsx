          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-sm font-medium">{accuracyPercentage || 0}%</span>
              </div>
              <Progress 
                value={accuracyPercentage || 0} 
                className={`h-2 ${
              ...
              <div className="flex justify-between mt-1">
                <div className="flex items-center">
                  {[...Array(Math.floor((accuracyPercentage || 0) / 20))].map((_, i) => (
                    <Star key={i} className="h-4 w-4 text-yellow-500 fill-yellow-500" />
                  ))}
                  {[...Array(5 - Math.floor((accuracyPercentage || 0) / 20))].map((_, i) => (
                    <Star key={`empty-${i}`} className="h-4 w-4 text-gray-300" />
                  ))}
                </div>
                <span className="text-xs text-gray-500">
                  {(accuracyPercentage || 0) >= 70 
                    ? 'Excelente!' 
                    : (accuracyPercentage || 0) >= 50 
                      ? 'Bom trabalho!' 
                      : 'Continue praticando!'}
                </span>
              </div>
