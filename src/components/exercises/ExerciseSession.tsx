...
const ExerciseSession: React.FC<ExerciseSessionProps> = ({ 
    ...
    const fetchExercises = async () => {
      try {
        ...
        if (categoryId.startsWith('virgula-') || categoryId === 'prova-virgula' || 
          ...
          // Função para verificar e remover exercícios duplicados do array original
          const removeDuplicateExercises = (array: Exercise[]): Exercise[] => {
            // Usar um Map para rastrear exercícios por questão
            const uniqueExercisesMap = new Map<string, Exercise>();
            
            // Adicionar apenas exercícios com questões únicas ao Map
            array.forEach(exercise => {
              if (!uniqueExercisesMap.has(exercise.question)) {
                uniqueExercisesMap.set(exercise.question, exercise);
              }
            });
          ...
          // Garantir que não há exercícios com perguntas duplicadas antes de processar
          const uniqueExercises = removeDuplicateExercises(fetchedExercises);
          console.log(`[ExerciseSession] Array original tinha ${fetchedExercises.length} exercícios, após remover duplicatas: ${uniqueExercises.length}`);
          
          const orderedExercisesWithShuffledOptions = uniqueExercises.slice(0, maxExercisesToUse).map(exercise => {
          ...
          // Para outros módulos, garantir que não haja repetições
          ...
          const shuffleWithoutRepetition = (array: Exercise[], limit: number) => {
            // Garantir que não há exercícios com perguntas duplicadas no array original
            const uniqueArray = removeDuplicateExercises(array);
            console.log(`[ExerciseSession] Array original tinha ${array.length} exercícios, após remover duplicatas: ${uniqueArray.length}`);
            
            // Criar uma cópia do array sem duplicatas
            const originalArray = [...uniqueArray];
            // Array que conterá os exercícios selecionados
            const selectedExercises: Exercise[] = [];
            
            // Selecionar elementos aleatórios até atingir o limite ou esgotar o array original
            while (selectedExercises.length < limit && originalArray.length > 0) {
              // Escolher um índice aleatório
              const randomIndex = Math.floor(Math.random() * originalArray.length);
              // Adicionar o exercício ao array de selecionados
              selectedExercises.push(originalArray[randomIndex]);
              // Remover o exercício do array original para evitar repetições
              originalArray.splice(randomIndex, 1);
            }
            
            // Embaralhar as opções de cada exercício selecionado
          ...
          // Aplicar o embaralhamento sem repetições
          const uniqueExercises = shuffleWithoutRepetition(fetchedExercises, maxExercises);
          console.log(`[ExerciseSession] Selecionados ${uniqueExercises.length} exercícios únicos para ${categoryId}`);
  ...
  const handleEndSession = async () => {
    ...
    if (user) {
      ...
      // Para módulos de palavras repetitivas, sempre usar 10 como número total
      const isPalavrasRepetitivas = categoryId.includes('palavras-repetitivas') || 
                                  categoryId === 'substituicao-palavras-comuns' || 
                                  categoryId === 'verbos-comuns' || 
                                  categoryId === 'conectivos-transicao' || 
                                  categoryId === 'palavras-imprecisas' || 
                                  categoryId === 'reducao-redundancias' ||
                                  categoryId === 'expressoes-argumentativas' ||
                                  categoryId === 'conclusoes-redacao';
      
      // Usar totalExercises conforme a categoria
      const finalTotalExercises = isProvaFinal ? 30 : (isPalavrasRepetitivas ? 10 : completedExercises.length);
  ...
  const handleReturnToModules = () => {
    // 1. Exercícios de pontuação (vírgula, ponto e vírgula, dois pontos, parênteses)
    // sempre retornam para /pontuacao (já configurado)
    if (categoryId.startsWith('virgula') || 
        categoryId === 'prova-virgula' || 
        categoryId.startsWith('ponto-e-virgula') || 
        categoryId.startsWith('ponto-virgula') || 
        categoryId === 'prova-ponto-e-virgula' || 
        categoryId.startsWith('dois-pontos') || 
        categoryId === 'prova-dois-pontos' || 
        categoryId.startsWith('parenteses') || 
        categoryId === 'prova-parenteses') {
      navigate('/pontuacao');
      return;
    }
    ...
    // 3. Exercícios de palavras repetitivas
    // sempre retornam para /palavras-repetitivas-pont
    if (categoryId === 'substituicao-palavras-comuns' || 
        ...
        categoryId === 'expressoes-argumentativas' || 
        categoryId === 'conclusoes-redacao' || 
        categoryId === 'prova-final-palavras-repetitivas') {
      navigate('/palavras-repetitivas-pont');
      return;
    }
    
    // 4. Caso padrão - redireciona para /pontuacao
    navigate('/pontuacao');
  };
          ...
          <CardContent className="pt-6">
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-6">
                Você acertou {correctAnswers} de {categoryId.includes('palavras-repetitivas') || 
                                              categoryId === 'substituicao-palavras-comuns' || 
                                              categoryId === 'verbos-comuns' || 
                                              categoryId === 'conectivos-transicao' || 
                                              categoryId === 'palavras-imprecisas' || 
                                              categoryId === 'reducao-redundancias' ||
                                              categoryId === 'expressoes-argumentativas' ||
                                              categoryId === 'conclusoes-redacao' ? 10 : exercises.length} questões.
              </p>
